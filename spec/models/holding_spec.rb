...
RSpec.describe Holding, type: :model do
  context 'validations' do
    ...
    it 'is valid with zero total_shares_owned' do
      holding = Holding.new(stock_name: 'AAPL', total_shares_owned: 0, average_buy_price: 150.0, total_value: 0.0, user: User.new)
      expect(holding).to be_valid
    end

    it 'is valid with zero average_buy_price' do
      holding = Holding.new(stock_name: 'AAPL', total_shares_owned: 10, average_buy_price: 0.0, total_value: 0.0, user: User.new)
      expect(holding).to be_valid
    end

    it 'is valid with zero total_value' do
      holding = Holding.new(stock_name: 'AAPL', total_shares_owned: 10, average_buy_price: 150.0, total_value: 0.0, user: User.new)
      expect(holding).to be_valid
    end
  end
end
